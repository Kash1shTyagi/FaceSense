...
namespace GMM
{
     ...
     */
    template <typename T>
    inline void invertMatrix(const std::vector<std::vector<T>> &mat,
                             ...
                             T &det)
    {
        ...
        for (size_t i = 0; i < k; ++i)
        {
            T d = mat[i][i];
            if (d <= T{})
                throw std::runtime_error("Non-positive variance in diagonal GMM");
            mat_inv[i][i] = T{1} / d;
            det *= d;
        }
    }
    ...
    public:
        ...
        void fit(const std::vector<std::vector<T>> &data, int max_iters = 100, T tol = static_cast<T>(1e-4))
        {
            size_t N = data.size();
            if (N == 0)
                throw std::runtime_error("No data for GMM fit");
            ...
            for (int iter = 0; iter < max_iters; ++iter)
            {
                ...
                for (size_t m = 0; m < M; ++m)
                {
                    ...
                    for (size_t i = 0; i < N; ++i)
                        N_m += gamma[i][m];
                    if (N_m <= T{})
                        N_m = std::numeric_limits<T>::epsilon();
                    // Update weight
